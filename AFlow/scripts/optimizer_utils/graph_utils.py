...
class GraphUtils:
    ...
    def load_graph(self, round_number: int, workflows_path: str):
        ...
        # 获取绝对路径
        workflows_path_abs = os.path.abspath(workflows_path)

        # 将workflows_path的父目录添加到sys.path，这样Python可以导入
        parent_dir = os.path.dirname(workflows_path_abs)
        if parent_dir not in sys.path:
            sys.path.insert(0, parent_dir)

        # 使用相对于parent_dir的模块名
        workflows_dirname = os.path.basename(workflows_path_abs)
        graph_module_name = f"{workflows_dirname}.round_{round_number}.graph"
        ...
        try:
            ...
            logger.error(f"Attempted module: {graph_module_name}")
            logger.error(f"Parent dir in sys.path: {parent_dir}")
            raise
